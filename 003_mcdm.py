...
rate = np.array(
    [[24, 5, 2, 20, 3], [12, 2, 1, 13, 2], [6, 3, 2, 15, 2], [12, 7, 3, 19, 4]]
)
...
decision_matrix = pd.DataFrame(
    data=rate,
    index=["A", "B", "C", "D"],
    columns=["opening_hours", "location", "equipment", "membership_fee", "cleanliness"],
)
...
result_index = pd.DataFrame(data=result, index=["A", "B", "C", "D"], columns=["value"])
result_sorted = result_index.sort_values(by=["value"], ascending=False)
ax = result_sorted["value"].plot(
    kind="bar", xlabel="Fitness Center", ylabel="Value", rot=0, colormap="coolwarm"
)
ax.figure.savefig("mcdm.png")
